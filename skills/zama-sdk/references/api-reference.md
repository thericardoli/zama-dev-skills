...
官方 GitBook 文档、示例项目和 API report 偶尔会有短暂不同步。遇到冲突时，以当前安装包的 `package.json` exports、`dist/**/*.d.ts`、以及 `packages/*/src` 源码为准。
...
| `useConfidentialBalance` / `useConfidentialBalances` | 解密展示当前 signer 的单个或多个 confidential balances。任意 owner 使用 core token 或 query factory。 | `useConfidentialBalance({ tokenAddress })`、`useConfidentialBalances({ tokenAddresses })` | `rg -n "useConfidentialBalance|useConfidentialBalances|UseConfidentialBalance" node_modules/@zama-fhe/react-sdk/dist/index.d.ts` |
